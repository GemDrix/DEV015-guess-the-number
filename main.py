...
jugadora_adivinando = True  # DEJARLO EN UNA FUNCION  "GAME"
turno_jugadora = True

while jugadora_adivinando:
    if turno_jugadora:

        # CAMBIAR Y DARLE MAYOR SEGURIDAD AL INPUT
        intento_jugadora = int(input("Adivina el número: "))

        if intento_jugadora < 1 or intento_jugadora > 100:
            print("Por favor, ingresa un número entre 1 y 100.")

        if intento_jugadora == random_number:  # ABSTRAER EN UNA FUNCION
            print("¡Felicidades! Has adivinado el número.")
            # termina turno de la jugadora.    #UTILIZAR UNA CONDICION Y NO EL BREAK ( CONDICION EN LA LINEA 12)
            break
        elif intento_jugadora < random_number:
            print("El número secreto es mayor.")
        else:
            # COLOCAR EN UNA FUNCION VERDADERO Y FALSO
